...
# Function to summarize metrics by article title
summarize_article_metrics <- function(metric_column, metric_name) {
  dta %>%
    group_by(TITLE) %>%
    summarise(metric = sum({{ metric_column }}, na.rm = TRUE)) %>%
    mutate(percent = round(metric / sum(metric) * 100, 2)) %>% 
    arrange(desc(metric)) %>%
    slice(1:1000) %>%  # Select top 1000 articles
    rename(!!metric_name := metric)
}

# Articles by reach
articles_reach <- summarize_article_metrics(REACH, "reach")
datatable(articles_reach, options = list(scrollX = TRUE, scrollY = "500px"))

# Articles by virality
articles_virality <- summarize_article_metrics(VIRALITY, "virality")
datatable(articles_virality, options = list(scrollX = TRUE, scrollY = "500px"))

# Articles by engagement rate
articles_engage <- summarize_article_metrics(ENGAGEMENT_RATE, "engagement_rate")
datatable(articles_engage, options = list(scrollX = TRUE, scrollY = "500px"))

# Articles by interactions
articles_interaction <- summarize_article_metrics(INTERACTIONS, "interactions")
datatable(articles_interaction, options = list(scrollX = TRUE, scrollY = "500px"))
```
