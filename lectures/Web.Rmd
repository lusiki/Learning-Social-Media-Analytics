...
# Filter top 100 articles by reach, including FROM_SITE and URL columns
top_articles_by_reach <- dta %>%
  group_by(TITLE, FROM_SITE, URL) %>%
  summarise(reach = sum(REACH, na.rm = TRUE)) %>%
  arrange(desc(reach)) %>%
  slice(1:100)  # Select top 100 articles

# Display the result in a datatable
datatable(top_articles_by_reach, options = list(scrollX = TRUE, scrollY = "500px"))
...
top_articles_by_reach <- dta %>%
  ...
  summarise(
    ...
    engagement_rate = sum(ENGAGEMENT_RATE, na.rm = TRUE),
    interactions = sum(INTERACTIONS, na.rm = TRUE)
  ) %>%
  arrange(desc(reach)) %>%
  slice(1:100)  # Select top 100 articles by reach
