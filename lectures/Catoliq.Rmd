...
pravno <- enframe(politika_root, name = "name", value = "root")
...
# Filter Catholiq podataka

```{r eval=F, message = FALSE, warning=FALSE}

# Assuming a data.table
setDT(dt)

dt <- all[SOURCE_TYPE == "web",-c(27:48)]
#dt <- dt[1:100000,]


words_vector <- generalno$root
...
check_matches <- function(text, words_vector) {
    ...
    patterns <- paste0("\\b", words_vector, "\\b")
    any(stri_detect_regex(text, patterns))
}



extract_sentences <- function(text, words) {
  sentences <- unlist(str_split(text, boundary("sentence")))
  
  # Loop through each word to find matched sentences
  all_matched_sentences <- sapply(words, function(word) {
    matched_sentences <- sentences[stri_detect_fixed(sentences, word)]
    paste(matched_sentences, collapse = " ")
  })
  
  # Combine all matched sentences
  paste(all_matched_sentences, collapse = " ")
}
...
generalno <- dt[has_match==T,]


#write_xlsx(generalno,"D:/LUKA/Academic/HKS/Projekti/Dezinformacije/Data/generalno.xlsx")
```
...
```{r}
proba <- dt[has_match==T]

#write_xlsx(proba,"D:/LUKA/Academic/HKS/Projekti/Dezinformacije/Data/proba.xlsx")
...
doprinos_sentimentu_full(n_tokenTidy,30)
```
...
domenaWords <- n_tokenTidy %>%
  filter(FROM %in% c("hkm.hr", "jutarnji.hr", "laudato.hr", "24sata.hr", "index.hr", "slobodnadalmacija.hr")) %>% 
...
process_batch <- function(batch) {
  batch %>%
    ...
    mutate(
      full_text = tolower(FULL_TEXT),
      match = str_c(pattern_institucije, collapse = '|'),
      key_word = str_extract_all(full_text, match)
    ) %>%
    filter(lengths(key_word) > 0) %>%
    unnest(key_word) %>%
