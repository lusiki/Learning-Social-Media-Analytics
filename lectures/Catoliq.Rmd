...
# Vectorized function to check for matches
check_matches <- function(text, pattern_imena) {
  any(stri_detect_fixed(text, pattern_imena, negate = FALSE))
}
...
for (i in 1:num_batches) {
  ...
  # Subset the data table for the current batch and apply the operations
  dt[start_idx:end_idx, `:=` (
    has_match = sapply(FULL_TEXT, check_matches, pattern_imena),
    matched_words = sapply(FULL_TEXT, function(x) paste(pattern_imena[stri_detect_fixed(x, pattern_imena)], collapse=", "))
  )]
