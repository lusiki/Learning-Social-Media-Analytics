...
proba <- n_tokenTidy %>% slice(300000:nrow(n_tokenTidy)) %>%
  mutate(
...
proba[, transformed_column := sapply(strsplit(sapply(word, write_tokens), "\t"), `[`, 2)]
...
batch_size <- 10000  # Adjust this to the size you want
number_of_batches <- ceiling(nrow(proba) / batch_size)
batched_data <- split(proba, seq(1, nrow(proba), by=batch_size))
...
#### Oblak riječi sa sentimentom (za sve native članke koje ste pronašli)
```{r echo=F, eval=T, message=F , warning= FALSE}
## ComparisonCloud
stemmed %>%
  inner_join(CroSentilex_Gold,by="word") %>% 
