...
n_tokenTidy %>%
  ...
  summarise(N = n()) %>%
  arrange(desc(N)) %>%
  filter(N > 150) -> n_tokenTidy

  
ggplot(n_tokenTidy, aes(reorder(word, N), N, fill = FROM)) +
...
domenaWords <- fb_tokenTidy_TopLike %>%
  filter(FROM %in% c("vecernji.hr", "jutarnji.hr", "slobodnadalmacija.hr", "Index.hr" )) %>% 
...
```{r nGRAMI,eval = F, echo = F, message=F, warning=F, fig.height=15, fig.width=15}
...
#  filter (!is.na(bigram)) %>%
  ...
  arrange(desc(tf_idf))
bigram_tf_idf %>%
  filter(FROM %in% c("vecenji.hr", "jutarnji.hr", "slobodnadalmacija.hr", "Index.hr")) %>%
  arrange(desc(tf_idf)) %>%
