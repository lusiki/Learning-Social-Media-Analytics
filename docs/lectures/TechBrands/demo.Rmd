...
subtitle: "<center><div class='mysubtitle'>Konkurenti: Bosch, Braun, Dyson</div></center>"
output:
  html_document:
    theme: yeti
    highlight: espresso
...
```{r toolkit, echo = FALSE, message=FALSE,warning=FALSE, eval=T}
...
<br><br>
- ukupno 2,677 objava, odnosno `r avg` objava dnevno
<br><br>
- ukupno 61,505 riječi u svim tekstovima
...
```{r eval = FALSE, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 10, fig.width=15}
...
<br>
- Najvažniji influenceri su AutoMarket.hr, Hanuma kocht i Slavica de Jong
...
- Riječi snaga, prilika, obitelj, dom, kvaliteta, ekološki najviše doprinose pozitivnom sentimentu na Facebooku
<br>
- Vidi grafikon za riječi koje doprinose negativnom i neutralnom sentimentu

<br>

<button class="btn btn-primary" data-toggle="collapse" data-target="#Blockinsert"> Grafikon: Riječi prema sentimentu na Facebook-u. </button>  
<div id="Blockinsert" class="collapse">

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height = 10, fig.width=15,fig.cap="Grafikon: Riječi prema sentimentu na Facebook-u."}
...
U ovom dijelu je pregled sadržaja za Bosch na Web-u i Phillips na Facebook-u.
...
<button class="btn btn-primary" data-toggle="collapse" data-target="#Block11"> Tablica: Najčešće riječi na Web-u. </button>  
...
- Najrelevantnije riječi na Facebooku su: slušalice,brijač, brada, kamenac, kosa i dr.
...
phillipstxt %>%
  ...
  mutate(word = gsub("\\d+", NA, word)) %>%
  mutate(word = gsub("^[a-zA-Z]$", NA, word)) %>%
  filter(!is.na(word)) -> philFB
```

<br>

<button class="btn btn-primary" data-toggle="collapse" data-target="#Block12"> Tablica: Najvažnije riječi na Facebook-u. </button>  
...
- Sonicare četkica postiže najveći doseg na Instagramu

<br>

<button class="btn btn-primary" data-toggle="collapse" data-target="#Block13">Tablica: Riječi koje postižu najveći doseg na Instagramu. </button>  
...
#  filter(word %in% c("xiaomi","samsung","vivo","huawei")) %>%
  ...
  summarise(Reach =  sum(REACH, na.rm = TRUE),
         ...
         #   Engagement =  sum(engagement, na.rm = TRUE),
          ...
          #  ShareCount =  sum(SHARE_COUNT, na.rm = TRUE),
            n = n()) %>%
  arrange(desc(Like))-> freqOFmention


freqOFmention %>%
  rename (Brand = word,
          'Number of mentions' = n) %>%
  kbl(caption = "Riječi koje postižu najveći doseg na Instagramu") %>%
    kable_paper(bootstrap_options = "striped",full_width = F, position = "left")  %>%
     scroll_box(width = "550px", height = "400px") 
...
- Juraj Šebalj je najvažniji Phillipsov influencer na Facebook-u. Druge influncere pogledaj u tablici.

<br>

<button class="btn btn-primary" data-toggle="collapse" data-target="#Block14">Tablica: Najvažniji influenceri na Facebook-u. </button>  
...
phillipstxt %>%
  ...
  filter(SOURCE_TYPE == "facebook" & kword == "phillips") %>%
  group_by(AUTHOR) %>%
  summarise(Like =  sum(LIKE_COUNT, na.rm = TRUE),
         #    FollowerCount =  sum(FOLLOWERS_COUNT, na.rm = TRUE),
         #   Engagement =  sum(engagement, na.rm = TRUE),
            Reach =  sum(REACH, na.rm = TRUE),
         #   Comment =  sum(COMMENT_COUNT, na.rm = TRUE),
...
freqOFmention %>%
 # rename (Brand = word,
 #         'Number of mentions' = n) %>%
  kbl(caption = "Najvažniji influenceri na Facebook-u") %>%
    kable_paper(bootstrap_options = "striped",full_width = F, position = "left")  %>%
